DROP TABLE IF EXISTS  mkt.mkt_imp_clk_cost_b;
CREATE TABLE IF NOT EXISTS mkt.mkt_imp_clk_cost_b AS
SELECT
    driver.biz_date,
    driver.biz_week,
    driver.biz_month,
...
(
    ...
    (
        SELECT DISTINCT
            biz_date,
            CASE
                WHEN MONTH(DATE(biz_date)) = 12 AND WEEKOFYEAR(DATE(biz_date)) = 1
        ...
        (
            ...
            WHERE channel IS NOT NULL AND biz_date >= '2025-04-01'
            UNION
            SELECT biz_date
            FROM ods.dwd_au_mkt_bing_campaign_report
            WHERE channel IS NOT NULL AND biz_date >= '2025-04-01'
        ) all_dates
    ) dt
    FULL JOIN
    (
        ...
        SELECT DISTINCT
            channel,
            CASE
              WHEN campaign_name = 'PMAX - Value' THEN 'max_value'
              WHEN campaign_name = 'NBR - Search - Master - Volume' THEN 'nbr_master_volume'
              WHEN campaign_name = 'NBR - Search - Payday Loans' THEN 'nbr-payday-loans'
              WHEN campaign_name = 'Search-Brand-03/04' THEN 'brand'
              WHEN campaign_name = 'NBR - Search - Instant Loans' THEN 'nbr-instant-loans'
              WHEN campaign_name = 'BRA - Search - Brand' THEN 'brand'
              WHEN campaign_name = 'NBR - Search - Competitors' THEN 'nbr-competitors'
              WHEN campaign_name = 'NBR - Search - Competitors | Value' THEN 'nbr-competitors'
              WHEN campaign_name = 'BRA - Search - Brand - All' THEN 'brand'
              WHEN campaign_name = 'Search Campaign_042826' THEN 'search_campaign_042826'  
          ELSE campaign_name
        END AS biz_campaign_name
    FROM ods.dwd_au_mkt_bing_campaign_report
    WHERE channel IS NOT NULL
...
AND driver.channel = r.attributed_utm
