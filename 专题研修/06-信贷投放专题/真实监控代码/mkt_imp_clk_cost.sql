DROP TABLE IF EXISTS mkt.mkt_imp_clk_cost;

CREATE TABLE IF NOT EXISTS mkt.mkt_imp_clk_cost AS
SELECT
    driver.biz_date,
    driver.biz_week,
    driver.biz_month,
...
(
    ...
    (
        ...
        WHERE channel IS NOT NULL
          ...
          AND account_id IN (7778217945, 9664233986, 4095144623)
    ) dt
    FULL JOIN
    (
        SELECT DISTINCT
...
(
    SELECT
        ...
        SUM(CASE WHEN account_id IN (7778217945) THEN cost*1.1
                WHEN account_id IN (9664233986, 4095144623) THEN cost*1.43 END) AS cost
    FROM edw.dws_mkt_media_campaign_hour_report_data
    WHERE channel IS NOT NULL
      AND biz_date >= '2025-04-01'
    GROUP BY 1,2,3
