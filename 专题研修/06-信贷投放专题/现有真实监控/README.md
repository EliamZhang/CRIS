...
| 文件 | 内容 | 看板中的用途 |
|------|------|--------------|
| MKT LeadsTrends_0417(1).xlsx | 月度线索量 × 渠道 | 渠道表现、Google 经营、告警阈值 |
| MKT Funded Trends_0417(1).xlsx | 月度放款量 × 渠道 | 同上 |
| MKT CPFL Trends_0417(1).xlsx | 月度 CPFL（总/Google/Bing/Lead Partner） | 投放与成本（CPFL 监控） |
| MKT Y26Green% Trends_0417(2).xlsx | 月度渠道绿/橙/红/NA 占比 | 渠道与信贷质量（真实部分） |
| MKT Y26KPI_0428.xlsx | 年度 KPI 参照（转化率/审批率/规模等） | 经营总览 KPI 参照、指标字典 |
...
| 模块 | 需要的数据 | 现状 |
|------|-----------|------|
| 1. 经营总览 | 5 份真实月报 + 获客明细（01） | ✅真实 + ⚠️明细待准备 |
| 2. 投放与成本 | 真实 CPFL ✅；首访 CPC/CPM/CTR（01+03 明细）；上游媒体账单（07） | ⚠️ 明细层缺失 |
| 3. 渠道与信贷质量 | 真实 Leads/Funded/Green% ✅；转化漏斗与风险（01） | ⚠️ 明细层缺失 |
| 4. ROI / LTV | 01 获客明细的价值字段（interest/fees/ltv 等） | ⚠️ 缺失 |
| 5. 自主归因 | 01 获客明细 + 02 触点明细（多触点路径、归因模型） | ⚠️ 缺失 |
| 6. Google 归因 | 真实 Google 列 ✅；04 回传日志 + 02 触点（资格与对账） | ⚠️ 明细层缺失 |
| 7. 渠道协同与蚕食 | 02 触点明细（跨渠道路径、First/Assist/Last） | ⚠️ 缺失 |
| 8. 增量与预算 | 06 实验 Holdout 结果（看板为手工输入，无需改 HTML） | ⚠️ 缺失 |
| 9. 回传监控 | 04 回传日志（链路、P90 时延、字段一致性） | ⚠️ 缺失 |
| 10. 系统健康 | 05 系统任务 SLA + 真实源快照 ✅ | ⚠️ 部分缺失 |
| 11. 告警中心 | 无需单独数据——由真实月报阈值（CPFL 涨幅/放款跌幅/Green 跌幅）与明细异常自动派生 | ✅ 派生 |
| 12. 单笔追踪 | 01+02+03+04 按 application_id / loan_id 联查 | ⚠️ 明细层缺失 |
...
> 明细层 01~05 可组装为 `schemaVersion=2` 的 JSON 快照导入看板；列名已与看板 JSON 契约字段名完全一致（camelCase），无需改名。
> 通用约定：日期为文本 `YYYY-MM-DD`；布尔写 `TRUE`/`FALSE`；空值留空（代表 null）；金额为 AUD 两位小数。

### 01_获客明细主表_Records_样例.xlsx（60 行）— 明细层核心主表
...
### 02_触点明细_Touches_样例.xlsx（78 行）— 自主归因 / 渠道协同
...
### 03_成本分摊明细_Costs_样例.xlsx（76 行）— 投放效率 / ROAS 分母
...
### 04_回传日志_Postbacks_样例.xlsx（153 行）— 回传监控 / Google 归因
...
### 05_系统任务SLA_Jobs_样例.xlsx（7 行）— 系统健康
...
### 06_增量实验Holdout_Experiment_样例.xlsx（4 行）— 增量与预算
...
### 07_媒体投放费用明细_MediaBilling_样例.xlsx（36 行）— 口径确认源表
...
| mediaCost | 媒体费（**媒体服务日账单口径**，与 03 的 Cohort 分摊口径不同，勿混算） |
...
| 口径项 | 含义 | 确认建议 |
|--------|------|----------|
| currency | 币种 | 由 07 文件的 currency 列确认（样例按 AUD） |
...
1. **P0 — 01 获客明细主表**：一张表打通转化漏斗、风险与用户、ROI/LTV、单笔追踪四个模块，价值最高
2. **P1 — 02 触点明细 + 03 成本分摊明细**：解锁自主归因、渠道协同、投放效率
3. **P2 — 04 回传日志**：解锁回传监控、Google 归因诊断
4. **P3 — 05 系统任务 SLA + 06 实验 Holdout + 07 媒体账单**：系统健康、增量验证、口径确认

准备完成后：将 01~05 按看板「数据接入」对话框的 schemaVersion=2 契约组装为 JSON 快照（列名一致，可直接转换），导入即可替换看板内的 Sample Data。
