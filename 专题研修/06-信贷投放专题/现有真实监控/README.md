...
>
> **加工层**见 `加工层/`：原始文件保持不变，加工层统一分析粒度、合并重复表，并生成可直接导入 HTML 的两套 JSON 快照。
...
| 文件 | 用途 |
|------|------|
| `加工层/01_监控加工模型_CuratedModel.xlsx` | 统一后的分析模型，含元数据、渠道映射、月度渠道指标、回传事件、实验与对账表 |
| `加工层/02_月度监控快照_monthly-monitor-v1_真实.json` | 由 01~05 生成，可直接导入 HTML 的真实月度快照 |
| `加工层/03_明细监控快照_schemaVersion2_样例.json` | 由 06~10 组装，可直接导入 HTML 的明细样例快照 |
| `加工层/00_加工层数据字典_CuratedDataDictionary.csv` | 加工层字段、类型、来源、键和 HTML 映射 |

加工层只用于分析与接口输出，不替代或覆盖原始 Excel。真实数据更新后应重新生成加工层，并核验数据质量检查结果。
...
> 明细层 06~10 可组装为 `schemaVersion=2` 的 JSON 快照导入看板。06、08、09、10 的业务字段与 JSON 合同一致；07 的 `touchId` 在组装时映射为 `records[].touches[].id`，07/08 按 `recordId` 嵌入 06 的 `touches`/`costs` 数组。
...
准备完成后：将 06~10 按看板「数据接入」对话框的 schemaVersion=2 契约组装为 JSON 快照，导入即可替换看板内的 Sample Data。组装时需完成 `touchId → id` 映射、07/08 子表嵌套，并附 meta 元数据块：
